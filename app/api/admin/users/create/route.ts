...
export async function POST(req: NextRequest) {
  try {
    ...
    const email_confirm: boolean = body?.email_confirm !== false; // default true
    if (!email) return new Response(JSON.stringify({ error: "email required" }), { status: 400 });

    const supa = supabaseAdmin();
    const { data, error } = await supa.auth.admin.createUser({ email, password, email_confirm });
    if (error) throw error;

    return new Response(
      JSON.stringify({ user: { id: data.user?.id, email: data.user?.email, email_confirmed_at: data.user?.email_confirmed_at } }),
